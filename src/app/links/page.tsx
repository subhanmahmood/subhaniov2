    ...
    return <>
        ...
        {session &&
            <div className='flex gap-1'>
                <Link prefetch={true} href="/links/add" className="flex">(<p className="text-gray-500 hover:text-gray-900 transition-colors duration-200 underline">Add</p>)</Link>
                <Link prefetch={true} href="/categories/edit" className="flex">(<p className="cursor-pointer text-gray-500 hover:text-gray-900 transition-colors duration-200 underline">Edit Category Order</p>)</Link>
            </div>
        }
        {categoriesWithLinks.map(linkGroup => {
            return (
                <div key={linkGroup.id}>
                    <p className="text-lg">{linkGroup.name}&nbsp;{session && (<Link prefetch={true} href={`/categories/edit/${linkGroup.id}`} className="text-gray-500 hover:text-gray-900 transition-colors duration-200 underline">Edit</Link>)}</p>
